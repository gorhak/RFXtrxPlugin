...
namespace SwitchKing.Server.Plugins.RFXtrx
{
        ...
        IDisposable
    {
        ...
        public Dictionary<string, Dictionary<string, string>> SensorMappings = new Dictionary<string, Dictionary<string, string>>();

        private Dictionary<string, Dictionary<string, object>> ReceivedList = new Dictionary<string, Dictionary<string, object>>();
        private Dictionary<string, Dictionary<string, object>> Transcievers = new Dictionary<string, Dictionary<string, object>>();
        private Dictionary<string, Dictionary<string, bool>> WhichOne = new Dictionary<string, Dictionary<string, bool>>();

        public Plugin()
        {
            var appConfig = ConfigurationManager.OpenExeConfiguration(Assembly.GetExecutingAssembly().Location);
            try { HostName      = appConfig.AppSettings.Settings["HostName"].Value; }       catch { HostName        = @"localhost"; }
            try { HostPort      = appConfig.AppSettings.Settings["HostPort"].Value; }       catch { HostPort        = @"10000"; }
            ...
            if (FilePath.StartsWith(@".\")) { FilePath = FilePath.Replace(@".\", appConfig.FilePath.Substring(0, (appConfig.FilePath.LastIndexOf(@"\") + 1))); }
            if (!(FilePath.LastIndexOf(@"\") == (FilePath.Length - 1))) { FilePath += @"\"; }
            //CommandThreshold = TimeSpan.FromMilliseconds(Convert.ToDouble(Threshold));
            if (!(StatusPath.LastIndexOf(@"\") == (StatusPath.Length - 1))) { StatusPath += @"\"; }
            
            tracelistener = new TextWriterTraceListener(System.IO.File.CreateText(@"C:\MyDataSource\RFXtrxPluginTrace.txt"));
            Debug.Listeners.Add(tracelistener); 

            try
            {
                ServerDB = new dBFunctions();
                //ServerDB = new dBFunctions(@"Data Source=" + FilePath + @"..\..\DB\switchKing.server.db3;");
                ...
                RFXComm.MessageArrived += new MessageHandler(RFXtrxEventReceived);
        ...
        public bool OnDeviceEventReceived(IDictionary<string, object> eventParams)
        {
            ...
            Trace(pid + " OnDeviceEventReceived", WriteToFile);
        ...
        public bool OnRawEventReceived(IDictionary<string, object> eventParams)
        {
            ...
            Trace("OnRawEventReceived " + eventParams.ToString(), WriteToFile);
        ...
        public bool OnSensorEventReceived(IDictionary<string, object> eventParams)
        {
            ...
            Trace("OnSensorEventReceived " + eventParams.ToString(), WriteToFile);
        ...
        public void OnCommand(int deviceId, string deviceName, int deviceActionId, string deviceActionName, int? dimLevel, string eventSource, out bool intercepted)
        {
            ...
            string pid = id.WithPrefix(DevicePrefix);
            Trace(pid + " OnCommand", WriteToFile);
            intercepted = false;
        ...
        private void SendRestCommand(string url)
        {
            Trace("SendRestCommand " + url.ToString(), true);
            {
        ...
        private string CreateUrl(string pid, string ActionName)
        {
        ...
        public static void Trace(string methodstr, bool WriteToFile)
        {
            if (WriteToFile)
            {
                var msg = String.Format(DateTime.Now.ToString() + ": " + methodstr);// + Environment.NewLine);
                tracelistener.WriteLine(msg);
                tracelistener.Flush();
                //System.IO.File.AppendAllText(@"C:\MyDataSource\Trace.txt", msg);
            }
        }
        ...
        private static void OnCommandToFile(int deviceId, string deviceName, int deviceActionId, string oldAction, int? dimLevel, string eventSource, string DebugFile, bool WriteToFile)
        {
            if (WriteToFile)
            {
                System.IO.File.AppendAllText(DebugFile,
                    "deviceId: " + deviceId.ToString() + Environment.NewLine +
                    "deviceName: " + deviceName.ToString() + Environment.NewLine +
                    "deviceActionId: " + deviceActionId.ToString() + Environment.NewLine +
                    "deviceActionName: " + oldAction.ToString() + Environment.NewLine +
                    "dimLevel: " + dimLevel.ToString() + Environment.NewLine +
                    "eventSource: " + eventSource.ToString() + Environment.NewLine +
                    Environment.NewLine);
            }
        }
        ...
        public void OnDataSourceValueSaved(int dataSourceId, string value, DateTime timestamp, DateTime localTimestamp, DateTime expires, bool isFailureValue, List<object> entityParams)
        {
            ...
            Trace(pid + " OnDataSourceValueSaved", WriteToFile);
            return;
        }
        ...
        public bool OnDataSourceValueSaving(int dataSourceId, string value, DateTime timestamp, DateTime localTimestamp, DateTime expires, bool isFailureValue, List<object> entityParams, out int dataSourceIdOut, out string valueOut, out DateTime timestampOut, out DateTime localTimestampOut, out DateTime expiresOut, out bool isFailureValueOut)
        {
            ...
            var id = dataSourceId.ToString();
            var pid = id.WithPrefix(SourcePrefix);
            Trace(pid + " OnDataSourceValueSaving", WriteToFile);
            valueOut = value;
        ...
        public void OnDeviceDeleted(int deviceId, string deviceNativeId, string deviceName, Dictionary<string, object> data)
        {
            ...
            string id = deviceId.ToString();
            string pid = id.WithPrefix(DevicePrefix);
            Trace(pid + " OnDeviceDeleted", WriteToFile);
        }
        ...
        public void OnDeviceEventEnqueued(int deviceId, string deviceNativeId, string deviceName, string deviceActionName, int? deviceActionDimLevel, Dictionary<string, object> data)
        {
            ...
            Trace(pid + " OnDeviceEventEnqueued", WriteToFile);
        }
        ...
        public void OnDeviceSaved(int deviceId, string deviceNativeId, string deviceName, bool deviceIsNew, Dictionary<string, object> data)
        {
            ...
            string id = deviceId.ToString();
            string pid = id.WithPrefix(DevicePrefix);
            Trace(pid + " OnDeviceSaved", WriteToFile);
        }
        ...
        public void OnDeviceSignalSent(int deviceId, string deviceNativeId, string deviceName, Dictionary<string, object> data)
        {
            ...
            string id = deviceId.ToString();
            string pid = id.WithPrefix(DevicePrefix);
            Trace(pid + " OnDeviceSignalSent", WriteToFile);
        }
        ...
        //    Trace(pid + " OnScenarioChanged", WriteToFile);
        ...
        //    Trace(pid + " OnSystemModeChanged", WriteToFile);
        ...
        public void OnDeviceChangeEventReceived(int deviceId, int changeEvent, int changeType)
        {
            ...
            string id = deviceId.ToString();
            string pid = id.WithPrefix(DevicePrefix);
            Trace(pid + " OnDeviceChangeEventReceived", WriteToFile);
        }
        ...
        public void OnDeviceEventReceived(int deviceId, string method, string data)
        {
            ...
            Trace(pid + " OnDeviceEventReceived " + method + " " + data, WriteToFile);
        }
        ...
        public void OnRawEventReceived(string data)
        {
            ...
            //string id = deviceId.ToString();
            //string pid = id.WithPrefix(DevicePrefix);
            Trace("OnRawEventReceived " + data, WriteToFile);
        }
        ...
        public void OnSensorEventReceived(string protocol, string model, int id, string dataType, string val, DateTime timestamp)
        {
            ...
            Trace(pid + " OnSensorEventReceived", WriteToFile);
        }

        #endregion ITelldusEventListener Members

        #endregion LittlePrince
    }
}
