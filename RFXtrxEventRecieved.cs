...
namespace SwitchKing.Server.Plugins.RFXtrx
{
        ...
        //IDisposable
    {
        ...
        public static void PidIn(string pid, string command)
        {
        ...
        private static bool NotInTreshold(string pid, string command)
        {
        ...
        private static bool InTreshold(string pid, string command)
        {
            return ((Plugin.ReceivedList[pid]["command"].ToString().Equals(command)) &&
                   (Convert.ToDateTime(Plugin.ReceivedList[pid]["timestamp"]).AddMilliseconds(Convert.ToDouble(Plugin.Threshold)) > DateTime.Now));
        }

        public static void PidOut(string pid, string command)
        {
        ...
        private static string AppendToHex(string[] buf, string[] msg)
        {
        ...
        public static void EventReceived(string message)
        {
            Plugin.TraceIt(message + ": " + "RFXtrxEventReceived", Plugin.WriteToFile);
            ...
            switch (msg[1])
            {
                case "01":  // IResponse ***
                    ...
                    //  0x5A = 868.35MHz FSK
                    //  0x5B = 868.95MHz
                    Plugin.TraceIt(message + ": " + "Interface Message", Plugin.WriteToFile);
                    #endregion
                    break;
                case "02":  // RXResponse ***
                    #region
                    ...
                    //  msg 0x03 = NAK, AC address zero in id1-id4 not allowed
                    {
                        ...
                        switch (RXResponse.SubType)
                        {
                            case "00":
                                #region
                                // 0x00 = error, receiver did not lock
                                Plugin.TraceIt(message + ": " + "error, receiver did not lock", Plugin.WriteToFile);
                                #endregion
                                break;
                            case "01":
                                #region
                                ...
                                // msg 0x03 = NAK, AC address zero in id1-id4 not allowed
                                {
                                    ...
                                    case "01":
                                        #region
                                        // ACK
                                        Plugin.TraceIt(message + ": " + "ACK", Plugin.WriteToFile);
                                        #endregion
                                        break;
                                    case "02":
                                    case "03":
                                        #region
                                        // NAK
                                        Plugin.TraceIt(message + ": " + "NAK", Plugin.WriteToFile);
                                        #endregion
                                        break;
                                    default:
                                        break;
                ...
                case "03":  // Undecoded RF Message
                    ...
                    Plugin.TraceIt(message + ": " + "Undecoded RF Message", Plugin.WriteToFile);
                    #endregion
                    break;
                case "10":  // Lighting1 *** X10, ARC, ELRO, Waveman, EMW200, IMPULS, RisingSun, Philips
                    ...
                    //Philips = unit 1 to 8
                    Plugin.TraceIt(message + ": " + "lighting1", Plugin.WriteToFile);
                    {
                ...
                case "11":  // Lighting2 ***
                    ...
                    switch (msg[2])
                    {
                        ...
                        case "02":  // ANSLUT
                            #region
                            Plugin.TraceIt(message + ": " + "lighting2", Plugin.WriteToFile);
                            {
                                RFXtrxLighting2 lighting2 = new RFXtrxLighting2(msg);
                                {
                                    string pid;
                                    if (Plugin.ServerDB.GetDeviceIDFromDeviceCode(lighting2.code, out pid))
                                    {
                                        if (Plugin.WhichOne.ContainsKey(pid) ? Plugin.WhichOne[pid]["RFXtrx"] : Plugin.Transcievers["RFXtrx"]["Default"].Equals(true))
                                        {
                                            PidIn(pid, lighting2.command);

                                            if (!InTreshold(pid, lighting2.command))
                                            {
                                                string url;
                                                url = Plugin.CreateUrl(pid, lighting2.level);
                                                Plugin.SendRestCommand(url);
                                            }

                                            PidOut(pid, lighting2.command);
                                        }
                                    }

                                    if (Plugin.ServerDB.GetDataSourceIDFromDeviceCode(lighting2.code, out pid))
                                    {
                                        if (Plugin.WhichOne.ContainsKey(pid) ? Plugin.WhichOne[pid]["RFXtrx"] : Plugin.Transcievers["RFXtrx"]["Default"].Equals(true))
                                        {
                                            PidIn(pid, lighting2.command);
                                            ...
                                            if (!InTreshold(pid, lighting2.command))
                                            {
                                                string url = Plugin.CreateUrl(pid, lighting2.command);
                                                Plugin.SendRestCommand(url);
                                            }
                ...
                case "12":  // Lighting3
                    ...
                    //9.8. 0x12, Lighting3
                    //9.8.1. 0x00-: Koppla
                    Plugin.TraceIt(message + ": " + "Lighting3", Plugin.WriteToFile);
                    #endregion
                    break;
                case "13":  // Lighting4
                    #region
                    //9.9. 0x13, Lighting4
                    //9.9.1. 0x00: PT2262
                    Plugin.TraceIt(message + ": " + "Lighting4", Plugin.WriteToFile);
                    #endregion
                    break;
                case "14":  // Lighting5
                    #region
                    ...
                    Plugin.TraceIt(message + ": " + "Lighting5", Plugin.WriteToFile);
                    #endregion
                    break;
                case "15":  // Lighting6
                    #region
                    ...
                    //        BYTE rssi : 4;    = 0x0 to 0xF = weak to strong (is 0x0 for transmitter command)
                    //} LIGHTING6;
                    Plugin.TraceIt(message + ": " + "Lighting6", Plugin.WriteToFile);
                    #endregion
                    break;
                case "18":  // Curtain1
                    #region
                    ...
                    Plugin.TraceIt(message + ": " + "Curtain1", Plugin.WriteToFile);
                    #endregion
                    break;
                case "19":  // Blinds1
                    #region
                    ...
                    Plugin.TraceIt(message + ": " + "Blinds1", Plugin.WriteToFile);
                    #endregion
                    break;
                case "20":  // Security1
                    #region
                    ...
                    Plugin.TraceIt(message + ": " + "Security1", Plugin.WriteToFile);
                    #endregion
                    break;
                case "28":  // Camera1
                    #region
                    ...
                    Plugin.TraceIt(message + ": " + "Camera1", Plugin.WriteToFile);
                    #endregion
                    break;
                case "30":  // Remote control and IR
                    ...
                    Plugin.TraceIt(message + ": " + "Remote control and IR", Plugin.WriteToFile);
                    #endregion
                    break;
                case "40":  // Thermostat1
                    #region
                    ...
                    Plugin.TraceIt(message + ": " + "Thermostat1", Plugin.WriteToFile);
                    #endregion
                    break;
                case "41":  // Thermostat2
                    #region
                    ...
                    Plugin.TraceIt(message + ": " + "Thermostat2", Plugin.WriteToFile);
                    #endregion
                    break;
                case "42":  // Thermostat3
                    #region
                    ...
                    Plugin.TraceIt(message + ": " + "Thermostat3", Plugin.WriteToFile);
                    #endregion
                    break;
                case "50":  // Temperature sensors
                    ...
                    #region
                    Plugin.TraceIt(message + ": " + "Temperature sensors", Plugin.WriteToFile);
                    {
                    ...
                    switch (TempSensor.SubType)
                    {
                        case "01": //0x01 = TEMP1 is THR128/138, THC138
                            #region
                            Plugin.TraceIt(message + ": " + "TEMP1 is THR128/138, THC138", Plugin.WriteToFile);
                            #endregion
                            break;
                        case "02": //0x02 = TEMP2 is THC238/268,THN132,THWR288,THRN122,THN122,AW129/131
                            #region
                            Plugin.TraceIt(message + ": " + "TEMP2 is THC238/268,THN132,THWR288,THRN122,THN122,AW129/131", Plugin.WriteToFile);
                            #endregion
                            break;
                        case "03": //0x03 = TEMP3 is THWR800
                            #region
                            Plugin.TraceIt(message + ": " + "TEMP3 is THWR800", Plugin.WriteToFile);
                            #endregion
                            break;
                        case "04": //0x04 = TEMP4 is RTHN318
                            #region
                            Plugin.TraceIt(message + ": " + "TEMP4 is RTHN318", Plugin.WriteToFile);
                            #endregion
                            break;
                        case "05": //0x05 = TEMP5 is La Crosse TX2, TX3, TX4, TX17
                            #region
                            Plugin.TraceIt(message + ": " + "TEMP5 is La Crosse TX2, TX3, TX4, TX17", Plugin.WriteToFile);
                            #endregion
                            break;
                        case "06": //0x06 = TEMP6 is TS15C
                            #region
                            Plugin.TraceIt(message + ": " + "TEMP6 is TS15C", Plugin.WriteToFile);
                            #endregion
                            break;
                        case "07": //0x07 = TEMP7 is Viking 02811
                            #region
                            Plugin.TraceIt(message + ": " + "TEMP7 is Viking 02811", Plugin.WriteToFile);
                            #endregion
                            break;
                        case "08": //0x08 = TEMP8 is La Crosse WS2300
                            #region
                            Plugin.TraceIt(message + ": " + "TEMP8 is La Crosse WS2300", Plugin.WriteToFile);
                            #endregion
                            break;
                        case "09": //0x09 = TEMP9 is RUBiCSON
                            #region
                            Plugin.TraceIt(message + ": " + "TEMP9 is RUBiCSON", Plugin.WriteToFile);
                            #endregion
                            break;
                        case "0A": //0x0A = TEMP10 is TFA 30.3133
                            #region
                            Plugin.TraceIt(message + ": " + "TEMP10 is TFA 30.3133", Plugin.WriteToFile);
                            #endregion
                            break;
                        default:
                            break;
                ...
                case "51":  // Humidity sensors
                    ...
                    //9.21.1. 0x01: TX3
                    Plugin.TraceIt(message + ": " + "Humidity sensors", Plugin.WriteToFile);
                    #endregion
                    break;
                case "52":  // Temperature and humidity sensors
                    ...
                    Plugin.TraceIt(message + ": " + "Temperature and humidity sensors", Plugin.WriteToFile);
                    #endregion
                    break;
                case "53":  // Barometric sensors
                    ...
                    Plugin.TraceIt(message + ": " + "Barometric sensors", Plugin.WriteToFile);
                    #endregion
                    break;
                case "54":  // Temperature, humidity and barometric sensors
                    ...
                    Plugin.TraceIt(message + ": " + "Temperature, humidity and barometric sensors", Plugin.WriteToFile);
                    #endregion
                    break;
                case "55":  // Rain sensors
                    ...
                    Plugin.TraceIt(message + ": " + "Rain sensors", Plugin.WriteToFile);
                    #endregion
                    break;
                case "56":  // Wind sensors
                    ...
                    Plugin.TraceIt(message + ": " + "Wind sensors", Plugin.WriteToFile);
                    #endregion
                    break;
                case "57":  // UV sensors
                    #region
                    ...
                    Plugin.TraceIt(message + ": " + "UV sensors", Plugin.WriteToFile);
                    #endregion
                    break;
                case "58":  // Date/time sensors
                    ...
                    //9.28.1. 0x01: DT1
                    Plugin.TraceIt(message + ": " + "Date/time sensors", Plugin.WriteToFile);
                    #endregion
                    break;
                case "59":  // Current sensors
                    ...
                    //9.29.1. 0x01: ELEC1
                    Plugin.TraceIt(message + ": " + "Current sensors", Plugin.WriteToFile);
                    #endregion
                    break;
                case "5A":  // Energy usage sensors
                    ...
                    Plugin.TraceIt(message + ": " + "Energy usage sensors", Plugin.WriteToFile);
                    #endregion
                    break;
                case "5B":  // Gas usage sensors
                    ...
                    Plugin.TraceIt(message + ": " + "Gas usage sensors", Plugin.WriteToFile);
                    #endregion
                    break;
                case "5C":  // Water usage sensors
                    ...
                    Plugin.TraceIt(message + ": " + "Water usage sensors", Plugin.WriteToFile);
                    #endregion
                    break;
                case "5D":  // Weighting scale
                    ...
                    Plugin.TraceIt(message + ": " + "Weighting scale", Plugin.WriteToFile);
                    #endregion
                    break;
                case "70":  // RFXsensor ***
                    #region
                    ...
                    switch (RFXsensor.SubType)
                    {
                        case "00": //0x00 = RFXSensor temperature
                            ...
                            //measured value in Celsius * 100.
                            //Bit 7 in msg1 is a sign bit. For a negative temperature this bit is 1.
                            Plugin.TraceIt(message + ": " + "RFXSensor temperature", Plugin.WriteToFile);
                            {
                        ...
                        case "01": //0x01 = RFXSensor A/D
                            ...
                            //msg1-msg2 for subtype 01 or 10
                            //measured value in mV
                            Plugin.TraceIt(message + ": " + "RFXSensor A/D", Plugin.WriteToFile);
                            #endregion
                            break;
                        case "02": //0x02 = RFXSensor voltage
                            ...
                            //msg1-msg2 for subtype 01 or 10
                            //measured value in mV
                            Plugin.TraceIt(message + ": " + "RFXSensor voltage", Plugin.WriteToFile);
                            #endregion
                            break;
                        case "03": //0x03 = RFXSensor message
                            ...
                            //0x0084 = no end of read signal Received from 1-Wire device
                            //0x0085 = 1-Wire scratchpad CRC error
                            Plugin.TraceIt(message + ": " + "RFXSensor message", Plugin.WriteToFile);
                            #endregion
                            break;
                        default:
                            break;
                ...
                case "71":  // RFXMeter
                    ...
                    Plugin.TraceIt(message + ": " + "RFXMeter", Plugin.WriteToFile);
                    #endregion
                    break;
                default:
                    #region
                    // Unknown message
                    Plugin.TraceIt(message + ": " + "Unknown message", Plugin.WriteToFile);
                    #endregion
                    break;
            }
        }
