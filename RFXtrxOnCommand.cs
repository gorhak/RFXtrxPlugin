                    ...
                    switch (type)
                    {
                        ...
                        case "37":      // Kappa, Self Learning Dimmer, arctech
                            ...
                            try
                            {
                                ...
                                {
                                    string code;
                                    if (ServerDB.GetDeviceCodeFromDeviceID(pid, out code))
                                    {
                                        {
                                            string description;
                                            if (type == "36" && ServerDB.GetDeviceDescriptionFromDeviceID(pid, out description))
                                            {
                                                if (description.Contains("ANSLUT")) { lighting2.SubType = "02"; }
                                            }
                                        }

                                        lighting2.command = deviceActionName;
