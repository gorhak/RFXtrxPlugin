        ...
        void comPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            switch (CurrentTransmissionType)
            {
                ...
                case TransmissionType.Hex:
                    ...
                    {
                        Plugin.TraceIt(queue.Count().ToString() + " Bytes thrown: " + "queue.Clear()", Plugin.WriteToFile);
                        queue.Clear();
                    }
                    TS = DateTime.Now;
                    ...
                    Plugin.TraceIt(String.Format(ByteToHex(comBuffer).ToString() + ": " + "comBuffer"), Plugin.WriteToFile);
                    ...
                    while (0 < queue.Count() && int.Parse(queue.Peek().ToString(), NumberStyles.HexNumber) < queue.Count())                      
                    {
                        ...
                        switch (length)
                        {
                            ...
                            case 2:
                            case 3:
                                Plugin.TraceIt(queue.Peek().ToString() + ": " + "Thrown", Plugin.WriteToFile);
                                queue.Dequeue();
                                break;
                            //case 4:
                            //case 5:
                            ...
                            default:
                                ...
                                Plugin.TraceIt(builder.ToString().ToUpper() + ": " + "Dequeued", Plugin.WriteToFile);
                                break;
                        }
                        if (!String.IsNullOrEmpty(builder.ToString()))
                        {
                            //string message = builder.ToString().ToUpper();
                            Plugin.TraceIt(builder.ToString().ToUpper() + ": " + "message", Plugin.WriteToFile);
                            //return the converted value
